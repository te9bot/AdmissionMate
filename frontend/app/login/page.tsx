...
export default function LoginPage() {
  ...
  const { requestOtp, verifyOtp, login, setPassword, logout } = useAuth();
  ...
  async function handleSignupDetails(e: React.FormEvent) {
    ...
    try {
      const res = await requestOtp(email, name || undefined);
      setDevCode(res.dev_code);
      setPendingSignupPassword(newPassword);
      setMode("otp-code");
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "Could not send code");
    } finally {
  ...
  async function handleVerify(e: React.FormEvent) {
    ...
    try {
      ...
      if (pendingSignupPassword) {
        await setPassword(pendingSignupPassword);
        ...
        {mode === "otp-code" && (
          <form className="mt-6 flex flex-col gap-3" onSubmit={handleVerify}>
            ...
            <button
              type="button"
              onClick={() => {
                const backMode = pendingSignupPassword ? "signup" : "otp-email";
                setPendingSignupPassword(null);
                setMode(backMode);
              }}
              className="text-xs font-medium text-white/70 hover:text-white"
